...
class IntCodeProgram:
    ...
    def run(self, inputs=None) -> list[int]:
        if inputs is None:
            inputs = []
        self.inputs = inputs
    ...
    def get_n_outputs(self, n) -> tuple[list[int], int]:
        outputs = []
        for _ in range(n):
    ...
    def get_one_output(self) -> tuple[int, int]:
        while True:
            opcode = self.memory[self.cursor] % 100
            params = self.memory[
                self.cursor + 1 : self.cursor + 1 + opcode_param_count[opcode]
            ]
            modes = [
                int(mode)
                for mode in reversed(
                    str(self.memory[self.cursor] // 100).zfill(len(params))
                )
            ]
            ...
            match opcode:
                case 1:  # add
                    ...
                    self.write_num(
                        destination, self.read_num(left_num) + self.read_num(right_num)
                    )
                case 2:  # multiply
                    left_num, right_num, destination = param_modes
                    self.write_num(
                ...
                case 4:  # output
                    ...
                    return self.read_num(to_return), ExitCode.HALTED
                case 5:  # jump-if-true
