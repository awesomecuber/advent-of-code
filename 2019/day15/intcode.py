...
class IntCodeProgram:
    ...
    def get_one_output(self) -> tuple[int | None, ExitCode]:
        ...
        while True:
            ...
            params = self._memory[self._cursor + 1 : self._cursor + 1 + opcode_param_count[opcode]]
            modes = [
                int(mode) for mode in reversed(str(self._memory[self._cursor] // 100).zfill(len(params)))
            ]
            ...
            match opcode:
                case 1: # add
                    ...
                    self._write_num(destination, self._read_num(left_num) \
                                                + self._read_num(right_num))
                case 2: # multiply
                    left_num, right_num, destination = param_modes
                    self._write_num(destination, self._read_num(left_num) \
                                                * self._read_num(right_num))
