...
for _ in range(1000):
    ...
    moon_velocities = [add_tuples(velocity, gravity) for velocity, gravity in zip(moon_velocities, gravities)]
    moon_positions = [add_tuples(position, velocity) for position, velocity in zip(moon_positions, moon_velocities)]
...
print(sum(
    energy(pos, vel) for pos, vel in zip(moon_positions, moon_velocities)
))