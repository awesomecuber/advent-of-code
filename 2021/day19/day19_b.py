    ...
    for other_scanner in orientations(relative_scanner):
        for delta in all_deltas(objective_scanner, other_scanner):
            ...
            if matches >= 6: # wont work with more???
                ...
                return set(tuple_add(other_point, delta) for other_point in other_scanner)
    return set()
...
while len(cur_objective_scanners) > 0:
    ...
    for objective_scanner, (i, relative_scanner) in product(cur_objective_scanners, enumerate(relative_scanners)):
        if i in to_remove:
            continue
...
for scanner1, scanner2 in combinations(scanner_locations, 2):
    man_dist = abs(scanner1[0] - scanner2[0]) + abs(scanner1[1] - scanner2[1]) + abs(scanner1[2] - scanner2[2])
