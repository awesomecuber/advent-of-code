...
for pattern in patterns:
    ...
    for n in range(1, len(pattern) // fourty_two_length):
        m = (len(pattern) - n * fourty_two_length) / (
            fourty_two_length + thirty_one_length
        )
        if m.is_integer():
            m = int(m)
            possible_counts.add((n, m))
    for n, m in possible_counts:
        fourty_two_partitions = [
            pattern[fourty_two_length * i : fourty_two_length * (i + 1)]
            for i in range(n + m)
        ]
        thirty_one_partitions = [
            pattern[thirty_one_length * i : thirty_one_length * (i + 1)]
            for i in range(n + m, n + 2 * m)
        ]
        if all(x in fourty_two_patterns for x in fourty_two_partitions) and all(
            x in thirty_one_patterns for x in thirty_one_partitions
        ):
